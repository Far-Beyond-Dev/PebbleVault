...
fn test_vault_manager_creation(db_path: &str) -> Result<(), String> {
    ...
    let backend = Box::new(SqliteDatabase::new(db_path).map_err(|e| e.to_string())?)
        as Box<dyn PersistenceBackend>;
    let vault_manager: VaultManager<TestCustomData> = VaultManager::new(backend)?;

    println!("{}", "VaultManager created successfully".green());

    // Assert that the VaultManager starts with no regions
    assert!(
        vault_manager.regions.is_empty(),
        "VaultManager should be created with empty regions"
    );
...
fn test_region_and_object_operations(db_path: &str) -> Result<(), String> {
    ...
    // Create a new VaultManager instance
    let backend = Box::new(SqliteDatabase::new(db_path).map_err(|e| e.to_string())?);
    let mut vault_manager: VaultManager<TestCustomData> = VaultManager::new(backend)?;

    // Assert that the VaultManager starts with no regions
    assert!(
        vault_manager.regions.is_empty(),
        "VaultManager should start with no regions"
    );
    println!(
        "{}",
        "VaultManager starts with no regions as expected".green()
    );
    ...
    println!(
        "Created cubic region with ID: {}",
        region_id.to_string().cyan()
    );

    // Assert that the VaultManager now has one region
    assert_eq!(
        vault_manager.regions.len(),
        1,
        "VaultManager should have one region after creation"
    );
    ...
    println!(
        "Added object 1 with UUID: {}",
        object1_uuid.to_string().cyan()
    );
    ...
    println!(
        "Added object 2 with UUID: {}",
        object2_uuid.to_string().cyan()
    );

    // Query the region to verify object addition
    let query_result =
        vault_manager.query_region(region_id, -50.0, -50.0, -50.0, 50.0, 50.0, 50.0)?;
    println!(
        "Query returned {} objects",
        query_result.len().to_string().cyan()
    );
    assert_eq!(query_result.len(), 2, "Query should return 2 objects");
    println!(
        "{}",
        "Query returned the expected number of objects".green()
    );

    // Print test passed message
    println!(
        "{}",
        "Region creation and object addition test passed".green()
    );
    Ok(())
}
...
fn test_querying_and_player_transfer(db_path: &str) -> Result<(), String> {
    ...
    // Create a new VaultManager instance
    let sqlite_backend = SqliteDatabase::new(db_path).map_err(|e| e.to_string())?;
    let mut vault_manager =
        VaultManager::new(Box::new(sqlite_backend)).map_err(|e| e.to_string())?;

    // Create two cubic regions
    let region1_id = vault_manager.create_or_load_region([0.0, 0.0, 0.0], 100.0)?; // 100x100x100 cube
    println!(
        "Created region 1 with ID: {}",
        region1_id.to_string().cyan()
    );
    let region2_id = vault_manager.create_or_load_region([200.0, 200.0, 200.0], 100.0)?; // 100x100x100 cube
    println!(
        "Created region 2 with ID: {}",
        region2_id.to_string().cyan()
    );
    ...
    let query_result =
        vault_manager.query_region(region1_id, -50.0, -50.0, -50.0, 50.0, 50.0, 50.0)?;
    println!(
        "Number of objects in region 1: {}",
        query_result.len().to_string().cyan()
    );
    for obj in &query_result {
        println!(
            "Object: UUID: {}, Type: {}, Custom Data: {:?}, Position: {:?}",
            obj.uuid.to_string().cyan(),
            obj.object_type,
            obj.custom_data,
            obj.point
        );
    ...
    assert_eq!(
        query_result.len(),
        1,
        "Query should return 1 object (player)"
    );
    println!(
        "{}",
        "Query returned the expected number of objects".green()
    ...
    let query_result1 =
        vault_manager.query_region(region1_id, -50.0, -50.0, -50.0, 50.0, 50.0, 50.0)?;
    println!(
        "Number of objects in region 1 after transfer: {}",
        query_result1.len().to_string().cyan()
    );
    assert_eq!(
        query_result1.len(),
        0,
        "Region 1 should be empty after transfer"
    );
    println!("{}", "Region 1 is empty as expected".green());

    // Query region 2 to verify player addition
    let query_result2 =
        vault_manager.query_region(region2_id, 150.0, 150.0, 150.0, 250.0, 250.0, 250.0)?;
    println!(
        "Number of objects in region 2 after transfer: {}",
        query_result2.len().to_string().cyan()
    );
    for obj in &query_result2 {
        println!(
            "Object in region 2: UUID: {}, Type: {}, Custom Data: {:?}, Position: {:?}",
            obj.uuid.to_string().cyan(),
            obj.object_type,
            obj.custom_data,
            obj.point
    ...
    assert_eq!(
        query_result2.len(),
        1,
        "Region 2 should contain the transferred player"
    );
    println!(
        "{}",
        "Region 2 contains the transferred player as expected".green()
    );

    // Verify player's new position
    let transferred_player = &query_result2[0];
    assert_eq!(
        transferred_player.point,
        [200.0, 200.0, 200.0],
        "Player should be at the center of region 2"
    );
...
fn test_persistence(db_path: &str) -> Result<(), String> {
    ...
    {
        let sqlite_backend = SqliteDatabase::new(db_path).map_err(|e| e.to_string())?;
        let boxed_backend: Box<dyn PersistenceBackend> = Box::new(sqlite_backend);
        let mut vault_manager: VaultManager<TestCustomData> =
            VaultManager::new(boxed_backend).map_err(|e| e.to_string())?;
    ...
    let backend = Box::new(SqliteDatabase::new(db_path).map_err(|e| e.to_string())?)
        as Box<dyn PersistenceBackend>;
    let vault_manager = VaultManager::<TestCustomData>::new(backend).map_err(|e| e.to_string())?;

    // Retrieve persisted objects
    let objects = vault_manager
        .persistent_db
        .get_points_within_radius(0.0, 0.0, 0.0, 100.0)
    ...
    for (i, obj) in objects.iter().enumerate() {
        println!(
            "Persisted object {}: UUID: {}, Type: {}, Custom Data: {:?}, Position: [{}, {}, {}]",
            i + 1,
            obj.id.unwrap().to_string().cyan(),
            obj.object_type,
            obj.custom_data,
            obj.x,
            obj.y,
    ...
    println!(
        "{}",
        "Correct number of objects loaded from persistent storage".green()
    );
...
fn test_with_arbitrary_struct(db_path: &str) -> Result<(), String> {
    ...
    let sqlite_backend = SqliteDatabase::new(db_path).map_err(|e| e.to_string())?;
    let mut vault_manager: VaultManager<ArbitraryGameObject> =
        VaultManager::new(Box::new(sqlite_backend)).map_err(|e| e.to_string())?;
    ...
    println!(
        "Added game object with UUID: {}",
        object_uuid.to_string().cyan()
    );
    ...
    assert_eq!(
        retrieved_object.uuid, object_uuid,
        "Retrieved object UUID should match"
    );
    assert_eq!(
        *retrieved_object.custom_data, *game_object,
        "Retrieved custom data should match original"
    );
    ...
    let backend = Box::new(SqliteDatabase::new(db_path).map_err(|e| e.to_string())?)
        as Box<dyn PersistenceBackend>;
    let new_vault_manager: VaultManager<ArbitraryGameObject> =
        VaultManager::new(backend).map_err(|e| e.to_string())?;

    let loaded_objects = new_vault_manager
        .persistent_db
        .get_points_within_radius(0.0, 0.0, 0.0, 100.0)
        .map_err(|e| format!("Failed to load objects from persistent database: {}", e))?;
    ...
    let loaded_custom_data: ArbitraryGameObject =
        serde_json::from_value(loaded_object.custom_data.clone())
            .map_err(|e| format!("Failed to deserialize loaded custom data: {}", e))?;
    assert_eq!(
        loaded_custom_data, *game_object,
    ...
    println!(
        "{}",
        "VaultManager with arbitrary struct test passed".green()
    );
    Ok(())
}
